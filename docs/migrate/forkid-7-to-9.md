...
4. Confirm onchain that fork 7 is running:
    ...
    ```sh
    kurtosis files download bdk-v6 genesis /tmp/fork-7-test
    cast call \
        --rpc-url "$(kurtosis port print bdk-v6 el-1-geth-lighthouse rpc)" \
...
3. Wait for the sequencer to halt and the verified batch to equal the latest batch and check the logs.
    ...
    ```sh
    kurtosis service logs bdk-v6 zkevm-node-sequencer-001 --follow
    ```
...
5. When those two numbers are the same, stop the services that are going to be upgraded:
    ...
    ```sh
    kurtosis service stop bdk-v6 zkevm-executor-pless-001
    kurtosis service stop bdk-v6 zkevm-node-aggregator-001
    kurtosis service stop bdk-v6 zkevm-node-eth-tx-manager-001
    kurtosis service stop bdk-v6 zkevm-node-l2-gas-pricer-001
    kurtosis service stop bdk-v6 zkevm-node-rpc-001
    kurtosis service stop bdk-v6 zkevm-node-rpc-pless-001
    kurtosis service stop bdk-v6 zkevm-node-sequence-sender-001
    kurtosis service stop bdk-v6 zkevm-node-sequencer-001
    kurtosis service stop bdk-v6 zkevm-node-synchronizer-001
    kurtosis service stop bdk-v6 zkevm-node-synchronizer-pless-001
    kurtosis service stop bdk-v6 zkevm-prover-001
    ```
...
3. Create the `PolygonValidiumStorageMigration` contract:
    ...
    ```sh
    export ETH_RPC_URL="$(kurtosis port print bdk-v6 el-1-geth-lighthouse rpc)"
    ger="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .polygonZkEVMGlobalExitRootAddress /opt/zkevm/combined.json" | tail -n +2)"
    pol="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .polTokenAddress /opt/zkevm/combined.json" | tail -n +2)"
    bridge="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .polygonZkEVMBridgeAddress /opt/zkevm/combined.json" | tail -n +2)"
    mngr="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .polygonRollupManager /opt/zkevm/combined.json" | tail -n +2)"
    forge create \
        --json \
        --private-key 0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625 \
...
4. Add a new rollup type to the rollup manager:
    ...
    ```sh
    genesis="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .genesis /opt/zkevm/combined.json" | tail -n +2)"
    cast send \
        --json \
        --private-key 0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625 \
        $mngr \
...
6. Update the rollup with the id:
    ...
    ```sh
    rollup="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .rollupAddress /opt/zkevm/combined.json" | tail -n +2)"
    cast send \
        --json \
        --private-key 0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625 \
        $mngr \
...
8. Set up the data availability protocol again:
    ...
    ```sh
    dac="$(kurtosis service exec bdk-v6 contracts-001 "jq -r .polygonDataCommittee /opt/zkevm/combined.json" | tail -n +2)"
    cast send \
        --json \
        --private-key "0x12d7de8621a77640c9241b2595ba78ce443d05e94090365ab3bb5e19df82c625" \
