...
1. Extract the `gasTokenAddress` from the generated files:
    ...
    ```sh
    kurtosis service exec bdk-v6 contracts-001 "cat /opt/zkevm/create_rollup_parameters.json"
    ```

2. Run the following command to mint some L1 tokens.

    ```sh
    export gta=$(kurtosis service exec bdk-v6 contracts-001 "cat /opt/zkevm/create_rollup_parameters.json" | tail -n +2 | jq -r .gasTokenAddress)
    export l1_rpc_url=$(kurtosis port print bdk-v6 el-1-geth-lighthouse rpc)
    cast send \
...
```sh
kurtosis port print bdk-v6 zkevm-bridge-proxy-001 web-ui
```
...
2. Do the same for L2:
    ...
    ```sh
    echo \{\"network_name\": \"kurtosis_cdk_rollup\", \"new_rpc_url\": \"$(kurtosis port print bdk-v6 zkevm-node-rpc-001 http-rpc)\", \"chain_id\": $(yq .args.zkevm_rollup_chain_id params.yml), \"currency_symbol\": \"CDK\"\} | jq
    ```
...
4. After some time, the transaction should complete.
    ...
    !!! tip
        If that doesn't happen, it's possible the auto-claiming service on L2 is not funded. Send it some Ether on L2 by running a command like this:
            ...
            --rpc-url "$(kurtosis port print bdk-v6 zkevm-node-rpc-001 http-rpc)" \
...
# The bridge address
bridge_addr="$(kurtosis service exec bdk-v6 contracts-001 "cat /opt/zkevm/combined.json" | tail -n +2 | jq -r .polygonZkEVMBridgeAddress)"

# Grab the endpoints for l1 and the bridge service
l1_rpc_url=$(kurtosis port print bdk-v6 el-1-geth-lighthouse rpc)
bridge_api_url="$(kurtosis port print bdk-v6 zkevm-bridge-service-001 rpc)"
